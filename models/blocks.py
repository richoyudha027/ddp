...
class PlainBlock(nn.Module):
    def __init__(self, input_channels, output_channels, stride=1, kernel_size=3, 
        ...
        # FIX: treat 0.0 same as None (no dropout)
        do = Identity() if (dropout_prob is None or dropout_prob == 0.0) else nn.Dropout3d(p=dropout_prob, inplace=True)
...
class ResidualBlock(nn.Module):
    def __init__(self, input_channels, output_channels, stride=1, kernel_size=3, 
        ...
        if (input_channels != output_channels) or (stride != 1):
            ...
            # FIX: was "lambda x: x" which can't be pickled by torch.save
